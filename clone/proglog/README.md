...
### 1. 주고받는 데이터의 암호화
데이터를 암호화하여 주고 받으면 중간자 공격을 막을 수 있다. 대표적인 중간자 공격으로는 능동 도청(active eavesdropping)이 있다.
중간에서 대화를 가로채거나 둘 사이의 메세지를 악의적으로 변경할 수 있다. 밥이 앨리스에게 페이팔을 이용해 송금하려 할 때, 맬러리가 수신 계좌를 자신의 계좌로 바꿀 수 있다.
이 중간자 공격을 막아주는 가장 널리 쓰이는 암호화 방법은 TLS이다.

클라이언트-서버 통신은 TLS 핸드셰이크부터 시작한다. 과정은 다음과 같다.
1. 사용할 TLS 버전을 명시한다.
2. 사용할 cipher suite(암호화 알고리즘들의 모음)을 결정한다.
3. 서버의 개인 키와 인증 기관의 서명으로 서버를 인증한다.
4. 핸드셰이크가 끝나면 대칭 키 암호화를 위해 세션 키를 생성한다.


### 2. 클라이언트 인증
인증이란, 클라이언트가 누구인지 확인하는 것이다. 예를 들어 트위터는 클라이언트가 트윗을 포스팅할 때마다 트윗을 포스팅하는 사람이 본인인지 확인한다.

대부분의 웹 서비스는 TLS를 단방향 인증으로 서버만 인증한다. 클라이언트  인증은 클라이언트 인증은 애플리케이션에서 구현할 몫이며, 보통은 사용자명-비밀번호와 토큰의 조합으로 구현한다.
TLS 상호 인증(mutual authentication), 또는 양방향 인증 (two-way authentication)이라고도 한다. 이 경우에는 서버와 클라이언트 모두 인증서를 이용ㅇ해 자신을 인증해야 한다.
이는 효과적이고, 간단하며 이미 많이 적용되어있다.

### 3. 클라이언트 권한
인증(authentication)과 권한 결정(authorization)은 깊게 연관되며, 둘 다 auth라고 줄여 불리곤한다.
특정 계정의 소유자라고 인증되면 그 계정에서 할 수 있는 모든 활동 권한을 갖는다.

인증과 권한의 구분은 리소스의 접근을 공유하고 소유권의 레벨이 다양할 때 필요하다.
예를 들어, 앨리스가 로그의 소유자이면서 읽고 쓰기 접근 권한을 가지고 있지만, 밥은 읽을 권한만 가지는 식이다.
이런 경우에 세분화한 접근 제어 권한이 필요하다.

## TLS로 서버 인증하기
데이터를 암호화하고 서버를 인증할 때 TLS를 사용하는 방법에 대해서 알아보자.
...
# 메트릭
### 메트릭의 종류
1. 카운터: 실패한 요청의 개수와 같은 특정 이벤트 횟수를 의미 또는 처리한 젅체 바이트와 같은 숫자 => 단순한 요청 수는 쓸모가 없고, 초당, 분당 요청 수가 중요하다.
2. 히스토그램: 데이터 분포를 보여준다. 주로 요청의 기간과 크기를 백분위로 측정한다.
3. 게이지: 디스크 사용 비율이나 최대 허용 개수 대비 인스턴스의 갯수 같은 포화 유형 메트릭에 유용
4. 레이턴시: 서비스가 요청을 처리하는 데 걸리는 시간이다. 레이턴시가 치솟으면 시스템의 인스턴스를 고사양으로 변경(스케일 업), 또는 로드 밸런서에 인스턴스를 추가하여 (스케일 아웃)한다.
5. 트래픽: 서비스 요청량으로, 일반적인 웹 서비스라면, 보통 초당 요청 처리 갯수를 말함. 현재 규모를 보여주고 앞으로 확장 계획을 하는데 도움이 된다.
6. 에러: 서비스의 요청 처리 실패율이다. 내부 서버 에러가 특히 중요.
7. 포화도: 서비스 용량을 측정한다. 디스크나 메모리가 서비스가 사용하고 있는 양이 얼마되는지 확인할 필요가 있다.


# 서비스 디스커버리
## 서비스 디스커버리란,
서비스 디스커버리를 사용하면 클러스터에 노드를 자동으로 추가하고 삭제할 수 있다.
서비스 디스커버리는 분산 서비스에서 가장 멋진 기능으로, 머신이 다른 머신을 자동으로 찾아준다.

## 서비스 디스커버리를 사용하는 이유
서비스 디스커버리는 서비스에 연결하는 방법을 알아내는 과정이다. 서비스 디스커버리는 레지스트리 목록을 항상 최신 상태로 유지해야한다.
레지스트리는 서비스와 서비스 위치, 그리고 서비스의 이상 유무를 가진다.
